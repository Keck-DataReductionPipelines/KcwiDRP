...
;
; CALLING SEQUENCE:
;	KCWI_STAGE6DRR, Pparfname, Linkfname
;
; OPTIONAL INPUTS:
...
;-
pro kcwi_stage6drr,ppfname,linkfname,help=help,select=select, $
	...
	if keyword_set(proc_imgnums) then begin
		...
		printf,ll,'Using these drrs : ',rnums
	endif else $
		...
		if file_test(obfil) then begin
			...
			if ppar.clobber eq 1 or not file_test(ofil) then begin
				...
				if rnums[i] ge 0 then begin
					...
					mrfile = cdir + strmid(rrf,0,strpos(rrf,'.fit'))+ '_drr.fits'
					;
					...
					if file_test(mrfile) then begin
						...
						rinfile = kcwi_get_imname(ppar,rnums[i],'_img',/reduced)
