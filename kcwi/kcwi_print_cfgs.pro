...
pro kcwi_print_cfgs,kcfg,imsum, $
	...
	if keyword_set(outfile) then begin
		...
		printf,ol,'#  #/   N Bin AMPS SSM IFU GRAT FILT    Cwave JDobs         Expt Type     Imno   RA          Dec             PA    Object'
	endif
	;
	; header?
	if keyword_set(header) and not keyword_set(silent) then begin
		print,' SSM = Sky, Shuffle, Mask: 0 - no, 1 - yes'
		print,'   #/   N Bin AMPS SSM IFU GRAT FILT    Cwave JDobs         Expt Type     Imno   RA          Dec             PA    Object'
	endif
	;
	; current date
	cdate = 0.d0
	...
	for i=0,n-1l do begin
		...
		imsum = string(i+1,'/',n,kcfg[i].xbinsize,kcfg[i].ybinsize, $
			...
			format='(i4,a1,i4,2i2,1x,a-5,3i1,1x,a-3,1x,a-4,1x,a-4,1x,f8.1,f12.3,f7.1,1x,a-8,i5,2f13.8,2x,f7.2)')
		;
		; add object info
