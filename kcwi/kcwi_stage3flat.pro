...
;
; CALLING SEQUENCE:
;	KCWI_STAGE3FLAT, Procfname, Pparfname
;
; OPTIONAL INPUTS:
...
;-
pro kcwi_stage3flat,procfname,ppfname,help=help,verbose=verbose, display=display
	;
	; setup
	pre = 'KCWI_STAGE3FLAT'
	startime=systime(1)
	...
	for i=0,nproc-1 do begin
		...
		if file_test(obfil) then begin
			...
			if kpars[i].clobber eq 1 or not file_test(ofil) then begin
				...
				;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
				;
				; do we have a master flat file?
				do_flat = (1 eq 0)	; assume no to begin with
				if strtrim(kpars[i].masterflat,2) ne '' then begin
					;
