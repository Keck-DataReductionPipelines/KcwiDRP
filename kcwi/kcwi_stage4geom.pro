...
;
; CALLING SEQUENCE:
;	KCWI_STAGE4GEOM, Pparfname, Procfname
;
; OPTIONAL INPUTS:
...
;-
pro kcwi_stage4geom,ppfname,procfname,help=help,verbose=verbose, display=display
	;
	; setup
	pre = 'KCWI_STAGE4GEOM'
	startime=systime(1)
			...
			if ppar.clobber eq 1 or not file_test(ofil) then begin
				...
				if strtrim(kpars[i].geomcbar,2) ne '' and $
				   strtrim(kpars[i].geomarc,2) ne '' then begin
					...
					if do_direct then $
						gfile = cdir + strmid(arf,0,strpos(arf,'_int'))+'_dgeom.save' $
					else	gfile = cdir + strmid(cbf,0,strpos(cbf,'_int'))+'_geom.save'
					;
