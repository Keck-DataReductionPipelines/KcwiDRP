...
;
;	The Hmf file that is read in by KCWI_FIX_HEADERS has five SPACE
...
;-
pro kcwi_fix_headers,hmf,froot=froot,fdigits=fdigits,update=update, $
	verbose=verbose
;
; program setup
...
imgnos = lonarr(nf)
for i=0,nf-1 do begin
	fdecomp,flist[i],disk,dir,root,ext
	digs = strmid(root,strlen(froot))
	imgnos[i] = long(digs)
endfor
;
