...
;
; CALLING SEQUENCE:
;	KCWI_STAGE8DAR, Procfname, Pparfname
;
; OPTIONAL INPUTS:
...
;-
pro kcwi_stage8dar,procfname,ppfname,help=help,verbose=verbose,display=display
	;
	; setup
	pre = 'KCWI_STAGE8DAR'
	startime=systime(1)
	...
	;
	; log file
	lgfil = reddir + 'kcwi_stage8dar.log'
		...
		if file_test(obfil) then begin
			...
			if kpars[i].clobber eq 1 or not file_test(ofil) then begin
				...
				for j = 0,sz[2]-1 do begin
					...
					var[*,*,j] = fshift(var[*,*,j],xsh,ysh)
					msk[*,*,j] = fshift(msk[*,*,j],xsh,ysh)
				endfor
