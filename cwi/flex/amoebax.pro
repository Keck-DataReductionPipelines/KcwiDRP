...
Function AmoebaX, ftol, xtol, FUNCTION_NAME=func, FUNCTION_VALUE=y, $
...
if not keyword_set(scale) then scale = fltarr(ndim)+1.0
...
while ncalls le nmax do begin		;Each iteration
  ...
  pavg = total(p,2)/mpts
  psize = total(abs(p[*,ihi] - pavg)/abs(scale))/ndim

  if (rtol lt ftol OR ftol LE 0.0) and (psize lt xtol OR xtol LE 0.0) then begin ;Done?
