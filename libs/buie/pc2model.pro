...
;    pc2model
; PURPOSE: (one line)
;    Generate synthetic PSF images for the HST WFPC2 Planetary Camera.
; DESCRIPTION:
...
;  CCD data processing
; CALLING SEQUENCE:
;    pc2model, xsrc, ysrc, inten, filter, bmvnum, back, image
; INPUTS:
...
;  SAMPFACT  : Sample factor (default: 3).
...
;    Written by Peter L. Collins, Lowell Observatory, June, 2007.
;      Based on p6model.pro by Doug Loucks.
;    2007/6/26, PLC, add keywords GRID, PSFSIZE, Z4. Rename x,y inputs
;                    to xsrc, ysrc, to avoid JITTER clobbering.
;  2007/07/08, MWB, psf placement reworked (was busted from long ago).
;  2007/08/08, MWB, fixed minor bug when pasting psf near top and right edge
;  2007/09/05, MWB, changed to permit multiple calls on same output image.
;                      Added pixel response function.
;  2007/09/12, PLC, provide DISTORT flag to use undistorted input x,y
;                   to generate psf's in the usual distorted (instrumental)
;                   frame.
;
;-

pro pc2model, xsrc_in, ysrc_in, inten,  filter, bmvnum, back, in_image,$
             DISTORT=distort, $
             GRID=grid, $
             HSTPATH=hstpath,$
             JITTER=jitter_in,$
   ...
   if n_params() lt 7 then begin
      print, 'pc2model, xsrc, ysrc, inten, filter, bmvnum, back, image'
      return
   endif
   ...
   for j=0, n1-1 do begin
      ...
      hstpsf, xsrc[j], ysrc[j], date, filter, bmvnum, psf, xm, ym, $
              ...
              CAMERA=6, Z4=z4, GRID=grid,  VERBOSE=verbose
   ...
   ; Add effect of the pixel response function, here we assume that saturation
   ;   does not occur and we just blur by the standard convolution kernel.
   kernel=[[0.0125,0.050,0.0125],[0.05,0.75,0.05],[0.0125,0.05,0.0125]]
   image=convol(image,kernel)
